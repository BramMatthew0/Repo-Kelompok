...
if (empty($nama) || empty($no_hp) || empty($email) || empty($alamat) || empty($kecamatan) || empty($kelurahan) || empty($username) || empty($password)) {
...
$stmt = $conn->prepare("SELECT Username FROM pelanggan WHERE Username = ?");
...
$stmt = $conn->prepare("INSERT INTO Pelanggan (Nama, No_HP, Email, Alamat, Kecamatan, Kelurahan, Username, Password) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

$stmt->bind_param("ssssssss", $nama, $no_hp, $email, $alamat, $kecamatan, $kelurahan, $username, $hashed_password);
